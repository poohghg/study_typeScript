export {};
/**
 *단일 타입이 아닌 다양한 타입에서 작동하게 할수 있다. 제네릭을 통해 여러 타입의 컴포넌트나 자신만의 타입을 사용할 수 있습니다.
...
function testClass() {
  ...
  const sDatas = new DataStorage<string>();
  // sDatas.addItem(1);
...
function utility() {
  ...
  // 파셜타입은 우리가 만든 타입의 속성이 선택적으로 사용될수 있다는것을 명시한다.
  ...
  // 오직 읽기만 가능한 배열이다.
  const ages: Readonly<number[]> = [1, 2, 3, 4, 5];
  // ages.push([1]);
  // ages.
  console.log(ages);
