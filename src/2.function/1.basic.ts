...
function basicFunction() {
  ...
  function addTpye(num1: number, num2: number): number {
  ...
  function printLog(action: Function, num1: number, num2: number): void {
    console.log(action(num1, num2));
  }

  // 자바스크립트 메커니즘상 undefined을 반환하지만 타입스크립트에서
  // 리턴타입으로 undefined을 사용하려면 다음과 같이 사용하여야함.
  function printLog1(action: Function, num1: number, num2: number): undefined {
    console.log(action(num1, num2));
    return;
    // return undefined;
  }
...
function typeFunction() {
  ...
  let fwithTyps: (a: number, b: number) => number;
  // error
  // fwithTyps = 6;
  fwithTyps = add;
  ...
  // 화살표 함수
  let addN = (n1: number, n2: number): number => n1 + n2;
...
function callback() {
  ...
  // 인자의 함수 리턴값이랑은 상관이없다
  // 반환된 값이 사용되지 않도록 명시?
  // 타입스크립트는 일급함수의 반환타입에 크게 관여하지 않는다.
  // 콜백 함수는 자신이 전달되는 인수가 반환 값을 기대하지 않는 경우에도 값을 반환할 수 있다.
  function actionFunction(n1: number, n2: number, cb: (num: number) => void) {
    const result = n1 + n2;
    cb(result);
    // return r;
